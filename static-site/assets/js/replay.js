// Static-site adaptation of the Razor replay component. The only structural
// change vs the original is the data fetch: instead of hitting a Razor
// handler, we look the battle file up in battles.json and parse the JSONL.
import { fetchBattles, fetchBattleEvents, getQueryParam } from './data.js';

window.replay = function replay() {
  return {
    battleId: getQueryParam('id') ?? '',
    events: [],
    idx: 0,
    playing: false,
    speed: 1,
    ...
    buttonSeat: null, sbSeat: null, bbSeat: null,
    ...
    async load() {
      if (!this.battleId) { this.notFound = true; return; }
      const battles = await fetchBattles();
      const found = battles.find(b => b.battleId === this.battleId);
      if (!found) { this.notFound = true; return; }
      this.events = await fetchBattleEvents(found.file);
      document.title = `Battle ${this.battleId} · AgentBattle`;
      this.rebuild(true);
      this.loaded = true;
    },
    ...
    prev() { if (this.idx > 0) { this.idx--; this.rebuild(true); } },
    next() {
      if (this.idx < this.events.length - 1) {
        this.idx++;
        this._stepForward(this.events[this.idx]);
      ...
      switch (e.t) {
        ...
        case 'agent_thoughts': {
          ...
          this.thoughtsStream.push({
            id, seat: e.seat, handNo: e.hand_no, author,
            street: this._streetForCurrent(),
            text: (e.text || '').trim(), action: null,
          });
          break;
        }
        ...
        case 'agent_note': {
          ...
          this.thoughtsStream.push({
            id, seat: e.seat, handNo: e.hand_no, author, street: 'note',
            text: '📝 ' + (e.text || '').trim(),
        ...
        case 'hand_ended': {
          ...
          this.currentSeat = null;
          if (!silent && winners.length > 0) this._highlightWinners(winners);
          break;
        }
    ...
    _showToast(e) {
      ...
      this.toast = {
        visible: true, kind: e.action,
        icon: icons[e.action] || '·', who,
    ...
    get rankedStrengths() { return this._strengthsForVisibleSeats(); },
